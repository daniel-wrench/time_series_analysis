...
def plotz(tfiles, rfiles, name, plotpoints, timestamp):
    ...
    for flt, flr in zip(tfiles, rfiles):
        ...
        ax0.set_title(
            f"Statistics for time series of ${name}$",  #  beginning {str(dr.index[0])}
            fontsize=14,
        )
        ...
        # else:
        ax3.plot(d[name]["times"], d[name]["cr"], label="ACF")
        ...
        ax3.text(tcor * 1.1, 0.8, r"$t_{{corr}}$ = {0:.02f} s".format(tcor))
        ax3.set_xlabel("$\Delta$t (s)")
        # ax3.legend()

        # Plot the structure function sfn
        ax4.loglog(d[name]["sfn"].iloc[:, 2], label="S$^{(2)}$")
        yyy = d[name]["sfn"].iloc[:, 2].values[58]
        af.pltpwrl(50, yyy * 2.0, xi=1, xf=500, alpha=2.0 / 3, label="2/3", ax=ax4)
        ...
        # else:
        ax5.semilogx(d[name]["sdk"], label="$\kappa_{" + name + "}$")
